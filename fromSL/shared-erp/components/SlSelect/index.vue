<template>
    <div class="stl-big-data-select" v-clickoutside="handleBlur">
        <div class="selected-tags" @click="toggleShow" :class="{'active': showDropdown, 'disabled': disabled}">
            <span v-if="(!selectedItem || !selectedItem.length) && !keyName" class="placeholder">{{placeholder}}</span>
            <span v-if="collapseTags && selectedItem.length > 2">
                <span v-for="item in selectedItem.slice(0,2)" :key="item[value]" class="multiple">
                    {{item[label]}}
                    <i @click.stop="delItem(item[value])"
                        v-if="!curDefaultValues.includes(item[value]) && multiple && !disabled"></i>
                </span>
                <span class="multiple" v-if="selectedItem.length > 2">
            ...
            <input v-if="filterable && !disabled" @input="handlerFilter" ref="selectInput" class="sl-select-input"
                v-model="keyName">
        ...
        <ul class="select-dropdown" v-if="showDropdown">
            <RecycleScroller :items="curOptions" :item-size="30" :key-field="value" v-slot="{ item, index }"
